...
void main(){
    ...
    // float heightClamp = (heightRange[1] - heightRange[0]) * (posZ + heightRange[0]);
    float heightClamp = (posZ + heightMax) / (heightMax - heightMin);
    // heightClamp = posZ;

    // if(posZ > 0.0) frag_out = vec4(color1.r, color1.g, color1.b, 1.0);
    // else frag_out = vec4(color2.r, color2.g, color2.b, 1.0);
