...
int main(int argc, char** argv){
    ...
    GLFWwindow* window = glfwCreateWindow(windowWidth, windowHeight, "First Person", nullptr, nullptr);
	...
	std::string camFlash_vert = parentDir + "\\shaders\\CamFlash.vert";
    std::string camFlash_frag = parentDir + "\\shaders\\CamFlash.frag";
    GLuint camFlash_glsl = compileShaders(camFlash_vert, camFlash_frag);
	...
	glUseProgram(camFlash_glsl);
	CamFlash CamFlashX(camFlash_glsl);
    CamFlashX.initUniforms();
	...
	while(!glfwWindowShouldClose(window)){
		...
		mvMatrix = glm::translate(glm::mat4(1), cameraPos);

        for(unsigned int d = 0; d < MPerComponent.size(); d++){
            if(MPerComponent.at(d).renderParams[ShaderCtrlBit::drawable] == 1){
				CamFlashX.mvMatrix(mvMatrix * MPerComponent.at(d).relativePos);
				CamFlashX.mvpMatrix(mvMatrix * MPerComponent.at(d).relativePos * perspectiveMatrix);
				CamFlashX.nMatrix(glm::mat3(glm::transpose(glm::inverse(MPerComponent.at(d).relativePos))));
				CamFlashX.materialBlock(&MPerComponent.at(d).materialBlock);
