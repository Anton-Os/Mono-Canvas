...
void God_keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods){
	...
	if(Key::W && !Player::isGod) Player::pos.y += Player::mvSpeed;
	...
	if(Key::A && !Player::isGod) Player::pos.x -= Player::mvSpeed;
	...
	if(Key::S && !Player::isGod) Player::pos.y -= Player::mvSpeed;
	...
	if(Key::D && !Player::isGod) Player::pos.x += Player::mvSpeed;
...
int main(int argc, char** argv) {
	...
	GL4_BumpGrid FlatGrid(10.0, 196, 220, 196, 220);
	...
	while(!glfwWindowShouldClose(window)){
		...
		Idle.set_mvpMatrix(Player::perspectiveMatrix * Player::viewMatrix * FlatGrid.relMatrix);
		...
		midPointT_Three.draw(GL_TRIANGLES, 3);
		Idle.set_renderMode(2);
		midPointT_Pos.create(&proxMidPointT.pos[0], sizeof(float) * 3);
		midPointT_Pos.draw(GL_POINTS, 1);
		if(isUpper) Idle.set_renderMode(1);
		else Idle.set_renderMode(2);
		rightVertex_Pos.create(glm::value_ptr(rightVertex), sizeof(float) * 3);
		rightVertex_Pos.draw(GL_POINTS, 1);
